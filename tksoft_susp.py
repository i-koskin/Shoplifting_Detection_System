...
class UnBlurApp:
    """
    A GUI application for selective unblurring of tracked objects in video streams.

    This application uses YOLOv8 for object detection and tracking, providing
    dual-view display with original and processed video streams. Users can
    selectively unblur or blur specific tracked objects by their IDs.
    ...
    def __init__(self, root):
        """
        Initialize the UnBlurApp application.

        Args:
            root (tk.Tk): The main Tkinter window
        """
        self.root = root
        self.root.title("Track ID UnBlur Tool with Dual View")
        ...
        # Bind space key for pause/resume functionality
        ...
        self.blur_mode = True  # Default: blur all objects
        ...
        self.setup_ui()
        logger.info("UnBlurApp initialized successfully")

    def setup_ui(self):
        """Setup the user interface with video displays, track ID selector, and control buttons."""
        try:
            # Video display frame
            ...
            tk.Label(track_id_frame, text="Select Track IDs to Show (Unblur):",
            ...
            buttons = [
                ...
                ("Pause/Resume (Space)", self.toggle_pause),
                ("Blur All", self.enable_blur),
                ("Show Selected", self.disable_blur),
                ("Quit", self.quit_app)
            ]
    ...
    def toggle_pause(self, event=None):
        """
        Toggle video pause/resume state with space key.
    ...
    def enable_blur(self):
        """Enable blur mode (blur all objects)."""
        try:
            self.blur_mode = True
            self.status_label.config(text="Mode: Blur All Objects", fg="red")
            messagebox.showinfo(
                "Blur Mode", "All objects will be blurred. Check IDs to show specific ones.")
            logger.info("Blur mode enabled - blurring all objects")
        except Exception as e:
            logger.error(f"Error enabling blur mode: {e}")
            messagebox.showerror("Error", f"Failed to enable blur mode: {e}")

    def disable_blur(self):
        """Disable blur mode (show only selected objects, blur others)."""
        try:
            self.blur_mode = False
            self.status_label.config(
                text="Mode: Show Selected Objects Only", fg="green")
            messagebox.showinfo(
                "Show Mode", "Only selected IDs will be shown. Others will be blurred.")
            logger.info("Show mode enabled - showing only selected objects")
        except Exception as e:
            logger.error(f"Error disabling blur mode: {e}")
            messagebox.showerror("Error", f"Failed to disable blur mode: {e}")
    ...
    def start_video(self):
        ...
        try:
            ...
            video_path = "./test_videos/vid.mp4"  # Path to the test video file
            ...
            self.video_writer = cv2.VideoWriter("susp.mp4", fourcc,
    ...
    def process_video(self):
        ...
        - If blur_mode is True: blur all objects
        - If blur_mode is False: blur all except selected IDs
        """
        def update():
                ...
                if not self.paused:
                    ...
                    if results and results[0].boxes is not None and results[0].boxes.id is not None:
                        ...
                        for box, class_id, track_id in zip(boxes, class_ids, track_ids):
                            ...
                            # Apply blur based on current mode and selection
                            if not self.blur_mode and track_id in self.selected_ids:
                                # Show selected ID (unblurred) when in show mode
                                cv2.rectangle(frame, (x1, y1),
                                              (x2, y2), (0, 255, 0), 2)
                                cv2.putText(frame, f'ID:{track_id}', (x2 + 10, y1 + 15),
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2)
                            else:
                                # Blur the object (either all objects in blur mode, or unselected in show mode)
    ...
    def display_frame(self, annotated, original):
        """
        Display frames in the GUI labels.

        Args:
            annotated (np.ndarray): Annotated/processed frame
